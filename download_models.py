...
def download_and_extract():
    # 1. Check if models actually exist (specifically .pkl files, not just the folder with .json files)
    if os.path.exists('models') and any(f.endswith('.pkl') for f in os.listdir('models')):
        print("Models (.pkl) already exist. Skipping download.")
        return
