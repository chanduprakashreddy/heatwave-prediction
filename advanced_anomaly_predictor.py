...
def build_anomaly_prediction_features(df, temp_col="Temperature", 
    ...
    if clim_mean is None:
        ...
        doy_groups = temp.groupby(doy)
        clim_mean_series = doy_groups.mean()
        clim_mean = doy_groups.mean()
    else:
        clim_mean_series = df.index.dayofyear.map(clim_mean)
    
    if clim_std is None:
        doy = df.index.dayofyear
        clim_std = doy.map(temp.groupby(doy).std())
    else:
        clim_std = df.index.dayofyear.map(clim_std)
