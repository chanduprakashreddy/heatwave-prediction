...
def train_xgb_model(df, temp_col="Temperature", city_name="default", force_retrain=False):
    ...
    print(f"⚠️ WARNING: Model not found at {model_p}. Starting training (This may crash on free servers due to memory limits)...")
    ...
    try:
        ...
        search = RandomizedSearchCV(
            base_model, param_distributions=param_grid, n_iter=4, # Reduced n_iter from 10 to 4
...
def train_sarima_model(df, temp_col="Temperature", city_name="default", force_retrain=False):
    ...
    split_idx = int(len(df) * 0.9)
    y_train = df[temp_col].iloc[:split_idx].dropna()
    
    try:
        model = SARIMAX(y_train, order=(1, 0, 1), seasonal_order=(1, 0, 1, 7), 
                        enforce_stationarity=False, enforce_invertibility=False)
        model_fit = model.fit(disp=False)
        
        # We also need to "apply" the model to the full history so get_forecast() works correctly 
        # from the present day when predicting ahead.
        y_full = df[temp_col].dropna()
        model_fit_full = model_fit.apply(y_full)
        
        os.makedirs(MODELS_DIR, exist_ok=True)
        joblib.dump(model_fit_full, model_p)
        print(f"[{city_name}] SARIMA model trained.")
        return model_fit_full
    except Exception as e:
        print(f"[{city_name}] Failed to train SARIMA: {e}")
        return None
