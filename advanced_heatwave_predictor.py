...
def train_heatwave_classifier(df, temp_col="Temperature", city_name="default", 
    ...
    # Convert y_test to a numpy array to ensure compatibility with scikit-learn metrics
    y_test_np = np.atleast_1d(y_test)

    # Handle case where only one class is present in y_test (e.g., no heatwaves).
    # This would cause roc_auc_score to raise a ValueError.
    if len(np.unique(y_test_np)) < 2:
        roc_auc = 0.5  # Undefined, assume no-skill classifier
    else:
